...
class ExchangeDeclareTestCase(testcase.RabbitTestCase, unittest.TestCase):
    ...
    def _test_exchange_declare(self, exchange_name, type_name, durable=False, auto_delete=False):
        ...
        exchange = exchanges[full_exchange_name]

        # assert exchange has been declared witht the good arguments
        self.assertEqual(full_exchange_name, exchange['name'])
    ...
    def test_passive(self):
        @asyncio.coroutine
        def go():
            full_exchange_name = self.full_exchange_name('e')
            ...
            with self.assertRaises(exceptions.ChannelClosed):
                yield from channel.exchange_declare(full_exchange_name, 'direct', passive=True)
            # create exchange
            yield from self.exchange_declare('e', 'direct')
            # get info
            channel = yield from self.create_channel()
            frame = yield from channel.exchange_declare(full_exchange_name, 'direct', passive=True)
